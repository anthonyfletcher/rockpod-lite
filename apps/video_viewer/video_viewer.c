 ...
 *
 * Thread list:
 *  1) The main thread - Handles user input, vv_settings, basic playback control
 *     and USB connect.
 *
...
#include "tree.h"            /* tree_get_context/entries for next/prev file */
#include "fs_attr.h"        /* ATTR_DIRECTORY */
#include "root_menu.h"      /* GO_TO_* */
#include "video_viewer.h"   /* public entry point */

/* Full-screen loading splash art, shipped for the 320x240 iPods only. */
#if defined(HAVE_LCD_COLOR) && (LCD_WIDTH == 320) && (LCD_HEIGHT == 240)
#include "bitmaps/rockpodvideo.h"
#define VV_HAVE_SPLASH_BMP
#endif


/* button definitions */
#if (CONFIG_KEYPAD == IPOD_4G_PAD)
/* iPod Classic 6G/7G and iPod Video 5G/5.5G share this keypad. */
...
{
    lcd_fillrect(__X, __Y, __W, __H);
}

static void draw_hline(int x1, int x2, int y)
{
#ifdef LCD_LANDSCAPE
    lcd_hline(x1 + osd.x, x2 + osd.x, y + osd.y);
#else
    y = LCD_WIDTH - (y + osd.y) - 1;
    lcd_vline(y, x1 + osd.x, x2 + osd.x);
#endif
}
...
{
#ifdef LCD_LANDSCAPE
    lcd_vline(x + osd.x, y1 + osd.y, y2 + osd.y);
#else
    y1 = LCD_WIDTH - (y1 + osd.y) - 1;
    y2 = LCD_WIDTH - (y2 + osd.y) - 1;
    lcd_hline(y1, y2, x + osd.x);
#endif
}
...
{
    unsigned oldfg = lcd_get_foreground();
    ...
    lcd_set_foreground(osd.prog_fillcolor);
                                           ...
                                           int width, int height)
{
    ...
    lcd_mono_bitmap_part(src, src_x, src_y, stride, x, y, width, height);
...
static void draw_putsxy_oriented(int x, int y, const char *str)
{
    int mode = lcd_get_drawmode();
    lcd_set_drawmode(DRMODE_FG);
    lcd_putsxy(x + osd.x, y + osd.y, str);
    lcd_set_drawmode(mode);
}
...
{
    vo_lock();
    lcd_update_rect(fps.pf_x, fps.pf_y,
...
static void fps_init(void)
{
    ...
    lcd_getstringsize(FPS_DIMSTR, &fps.rect.r, &fps.rect.b);
...
static void osd_lcd_enable_hook(unsigned short id, void* param)
{
    (void)id;
    (void)param;
    button_queue_post(LCD_ENABLE_EVENT_1, 0);
}
#endif

/* Local replacements for the plugin-lib helper.c backlight helpers. */
static void vv_backlight_ignore_timeout(void)
{
...
static void vv_backlight_use_settings(void)
{
...
#if defined(HAVE_LCD_ENABLE) || defined(HAVE_LCD_SLEEP)
        add_event(LCD_EVENT_ACTIVATION, osd_lcd_enable_hook);
#endif
        /* Revert to user's backlight vv_settings */
        vv_backlight_use_settings();
    }
}
...
{
    if (vv_settings.backlight_brightness < 0)
        return;

    mpeg_backlight_update_brightness(
        video_on ? vv_settings.backlight_brightness : -1);
}
#else
...
static void osd_text_init(void)
{
    ...
    lcd_getstringsize(buf, &osd.time_rect.r, &osd.time_rect.b);
    ...
    lcd_getstringsize(" ", &spc_width, NULL);
    lcd_getstringsize(buf, &osd.vol_rect.r, &osd.vol_rect.b);
...
static void osd_refresh_background(void)
{
    ...
    unsigned bg = lcd_get_background();
    lcd_set_drawmode(DRMODE_SOLID | DRMODE_INVERSEVID);

#ifdef HAVE_LCD_COLOR
    /* Draw a "raised" area for our graphics */
    lcd_set_background(draw_blendcolor(bg, LCD_WHITE, 192));
    draw_hline(0, osd.width, 0);

    lcd_set_background(draw_blendcolor(bg, LCD_WHITE, 80));
    draw_hline(0, osd.width, 1);

    lcd_set_background(draw_blendcolor(bg, LCD_BLACK, 48));
    draw_hline(0, osd.width, osd.height-2);

    lcd_set_background(draw_blendcolor(bg, LCD_BLACK, 128));
...
static void osd_refresh_volume(void)
{
    ...
    snprintf(buf, sizeof (buf), "%d%s",
                 ...
                 sound_unit(SOUND_VOLUME));
    lcd_getstringsize(buf, &width, NULL);
...
static void osd_refresh_status(void)
{
    ...
    lcd_set_foreground(draw_blendcolor(lcd_get_background(),
                        LCD_BLACK, 96));
...
static void osd_refresh(int hint)
{
     ...
     * will restore them. */
    oldfg = lcd_get_foreground();
    oldbg = lcd_get_background();

    draw_setfont(FONT_UI);
    lcd_set_foreground(osd.fgcolor);
    lcd_set_background(osd.bgcolor);
...
/* Handle Fast-forward/Rewind keys using WPS vv_settings (and some nicked code ;)
...
static void osd_set_volume(int delta)
{
    ...
    /* Sync the global vv_settings */
    if (vol != global_status.volume)
...
{
    int next_action = (vv_settings.play_mode == 0) ? VIDEO_STOP : VIDEO_NEXT;
    ...
    if (osd_play(vv_settings.resume_time) < STREAM_OK) {
    ...
    while (stream_status() != STREAM_STOPPED)
    {
...
#endif
        {
            ...
            next_action = (vv_settings.play_mode == 0) ? VIDEO_STOP : VIDEO_NEXT;
...
#endif
        {
            ...
            if ((old_button | BUTTON_REL) == button) {
                ...
                if (vv_settings.play_mode == 0 || osd.curr_time >= 3*TS_SECOND) {
 ...
 * Mirrors the text/image viewers' rockpod splashes. */
static void vv_splash(const char *file)
{
#ifdef VV_HAVE_SPLASH_BMP
...
static struct viewport vv_vp;

/* Core entry point. Opens `file` (mpg/mpeg/mpv/m2v) in the full-screen video
 * player and returns a GO_TO_* code for the browser. */
int video_viewer(const char *file)
{
    ...
    /* Reclaim the audio buffer (most of RAM) for the decoder. */
    audio_hard_stop();

    push_current_activity(ACTIVITY_VIDEOVIEWER);
     ...
     * matching the text/image viewers. */
    viewportmanager_theme_enable(SCREEN_MAIN, false, &vv_vp);
    ...
    if (stream_init() < STREAM_OK) {
        ...
        splash(HZ*2, "Could not initialize streams");
    } else {
    ...
    /* Hand the screen back to the theme before handling any deferred USB
     * event so the USB screen draws normally. */
    ...
    return (mpeg_sysevent() == SYS_USB_CONNECTED) ? GO_TO_ROOT : GO_TO_PREVIOUS;
}
