...
/* core_alloc handle for the buffer that backs mpeg_alloc_init() */
static int mpeg_buffer_handle = 0;
...
int stream_init(void)
{
    ...
    /* Grab the largest free buffer for the decoder + PCM + disk buffers.
     * Playback is stopped by the caller, so this is most of RAM. */
    mpeg_buffer_handle = core_alloc_maximum(&memsize, NULL);
    if (mpeg_buffer_handle <= 0)
    {
        splash(HZ, "Out of memory in stream_init");
        return STREAM_ERROR;
    }
    mem = core_get_data(mpeg_buffer_handle);
...
void stream_exit(void)
{
    ...
    if (mpeg_buffer_handle > 0)
    {
        core_free(mpeg_buffer_handle);
        mpeg_buffer_handle = 0;
    }
}
